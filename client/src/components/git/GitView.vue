...
const mobileView = ref<'controls' | 'diff'>('controls');

function onResize() {
  isMobile.value = window.innerWidth < 768;
}

function selectFileAndShowDiff() {
  if (isMobile.value) mobileView.value = 'diff';
}
...
<template>
  ...
  <div v-if="!isMobile" class="flex h-full">
  ...
  <div v-else class="flex h-full flex-col">
    ...
    <div class="flex h-10 items-center gap-1 border-b border-border px-2">
      <button
        class="flex min-h-[44px] flex-1 items-center justify-center gap-1.5 rounded-md px-2 py-1.5 text-sm font-medium transition-colors"
        :class="mobileView === 'controls'
          ? 'bg-background text-foreground shadow-sm ring-1 ring-border/60'
          : 'text-muted-foreground hover:text-foreground'"
        @click="mobileView = 'controls'"
      >
      ...
      <Button variant="ghost" size="sm" class="h-8 w-8 p-0" @click="git.fetchStatus">
        <RefreshCw class="h-3.5 w-3.5" :class="{ 'animate-spin': git.loading }" />
      </Button>
    </div>
    ...
    <div v-if="mobileView === 'controls'" class="flex flex-1 flex-col overflow-hidden">
      ...
      <ScrollArea class="min-h-0 flex-1">
        ...
        <Collapsible v-model:open="showBranches" @update:open="onBranchesToggle">
          <CollapsibleTrigger class="flex w-full items-center gap-2 border-b border-border px-3 py-2 text-xs text-muted-foreground transition-colors hover:bg-muted/30 hover:text-foreground">
            <ChevronRight class="h-3 w-3 shrink-0 transition-transform duration-200" :class="{ 'rotate-90': showBranches }" />
            <GitBranch class="h-3.5 w-3.5" />
            <span>Branches</span>
            <Badge variant="outline" class="ml-auto text-[10px]">{{ git.branches.length }}</Badge>
          ...
          <CollapsibleContent>
            <div class="border-b border-border">
              <div class="flex items-center gap-1 p-2">
                <input v-model="newBranch" class="h-7 flex-1 rounded-md border border-input bg-background px-2 text-xs" placeholder="New branch name..." @keyup.enter="handleCreateBranch" />
                <Button variant="ghost" size="sm" class="h-7 w-7 p-0" :disabled="!newBranch.trim() || creatingBranch" @click="handleCreateBranch">
                  <Plus class="h-3.5 w-3.5" />
                </Button>
              </div>
              <div v-for="b in git.branches" :key="b.name" class="group flex cursor-pointer items-center gap-2 px-3 py-1.5 text-xs hover:bg-muted/30" :class="{ 'bg-primary/10 text-primary font-medium': b.current }" @click="!b.current && git.checkoutBranch(b.name)">
                <GitBranch class="h-3 w-3 shrink-0" />
                <span class="truncate">{{ b.name }}</span>
                <Badge v-if="b.current" variant="default" class="ml-auto text-[9px]">current</Badge>
                <Button v-if="!b.current" variant="ghost" size="sm" class="ml-auto h-5 w-5 p-0 opacity-0 group-hover:opacity-100" title="Delete branch" @click.stop="handleDeleteBranch(b.name)">
                  <Trash2 class="h-3 w-3 text-destructive" />
                </Button>
              </div>
            </div>
        ...
        <Collapsible v-model:open="showHistory" @update:open="onHistoryToggle">
          <CollapsibleTrigger class="flex w-full items-center gap-2 px-3 py-2 text-xs text-muted-foreground transition-colors hover:bg-muted/30 hover:text-foreground">
            <ChevronRight class="h-3 w-3 shrink-0 transition-transform duration-200" :class="{ 'rotate-90': showHistory }" />
            <History class="h-3.5 w-3.5" />
            <span>History</span>
          </CollapsibleTrigger>
          <CollapsibleContent>
            <div>
              <div v-for="entry in git.log" :key="entry.hash" class="cursor-pointer border-b border-border/50 px-3 py-2 transition-colors hover:bg-muted/30" @click="git.showCommit(entry.hash)">
                <p class="truncate text-xs font-medium text-foreground">{{ entry.message }}</p>
                <div class="mt-0.5 flex items-center gap-2 text-[10px] text-muted-foreground">
                  <code class="rounded bg-muted px-1 py-0.5">{{ entry.hash.slice(0, 7) }}</code>
                  <span>{{ entry.author_name }}</span>
              ...
              <div v-if="git.log.length === 0" class="px-3 py-4 text-center text-xs text-muted-foreground">No commits yet</div>
            </div>
          </CollapsibleContent>
        </Collapsible>
      </ScrollArea>
    ...
    <div v-else class="flex flex-1 flex-col overflow-hidden">
      <div class="flex h-9 items-center gap-2 border-b border-border px-3">
        <Button variant="ghost" size="sm" class="h-6 text-xs" :class="{ 'bg-muted': !git.showStagedDiff }" @click="git.fetchDiff(false)">
          Working
        </Button>
        <Button variant="ghost" size="sm" class="h-6 text-xs" :class="{ 'bg-muted': git.showStagedDiff }" @click="git.fetchDiff(true)">
          Staged
        </Button>
        <span v-if="git.selectedFile" class="ml-2 truncate font-mono text-[11px] text-muted-foreground">{{ git.selectedFile }}</span>
      </div>
      <DiffViewer class="flex-1" />
    </div>
  </div>
