...
onMounted(() => {
  cleanupCopy = setupCodeCopyHandler();
  onResize();
...
<template>
  <div class="flex h-full flex-col">
    ...
    <template v-else>
            ...
            <div class="flex items-center gap-2 border-b border-border px-3 py-2">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <span class="text-sm font-medium truncate">{{ store.currentRun.agentAProfile.name }}</span>
                  <Badge variant="outline" class="text-[10px] px-1.5 py-0" :class="roleColor(store.currentRun.agentAProfile.role)">Agent A</Badge>
                </div>
                <p class="text-[11px] text-muted-foreground truncate">{{ store.currentRun.agentAProfile.description }}</p>
              </div>
              <Loader2 v-if="store.isRunning && currentCycle?.status === 'agent_a_running'" class="h-4 w-4 animate-spin text-blue-400 shrink-0" />
            </div>
              ...
              <template v-for="cycle in store.currentRun.cycles" :key="cycle.cycleNumber">
                ...
                <template v-if="cycle.agentA.contentBlocks.length > 0">
                  <template v-for="(block, bIdx) in cycle.agentA.contentBlocks" :key="bIdx">
                    <div v-if="block.type === 'text' && block.text" :class="proseClasses" v-html="rendered(block.text)" />
                    <ToolView v-else-if="block.type === 'tool'" :tool-use="toToolUseInfo(block.toolCall)" />
                  </template>
                </template>
                <template v-else>
                  <div v-if="cycle.agentA.response" :class="proseClasses" v-html="rendered(cycle.agentA.response)" />
                  <div v-if="cycle.agentA.toolCalls.length > 0" class="space-y-1.5">
                    <ToolView v-for="tc in cycle.agentA.toolCalls" :key="tc.requestId" :tool-use="toToolUseInfo(tc)" />
                  </div>
                </template>
              </template>
            </div>
        ...
        <ResizablePanel :default-size="50" :min-size="25">
          <div class="flex h-full flex-col border-l-2 border-l-emerald-500/60">
            <div class="flex items-center gap-2 border-b border-border px-3 py-2">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <span class="text-sm font-medium truncate">{{ store.currentRun.agentBProfile.name }}</span>
                  <Badge variant="outline" class="text-[10px] px-1.5 py-0" :class="roleColor(store.currentRun.agentBProfile.role)">Agent B</Badge>
                </div>
                <p class="text-[11px] text-muted-foreground truncate">{{ store.currentRun.agentBProfile.description }}</p>
              </div>
              <Loader2 v-if="store.isRunning && currentCycle?.status === 'agent_b_running'" class="h-4 w-4 animate-spin text-emerald-400 shrink-0" />
            </div>
            <div ref="agentBRef" class="flex-1 overflow-y-auto p-3 space-y-3">
              <template v-for="cycle in store.currentRun.cycles" :key="cycle.cycleNumber">
                ...
                <template v-else>
                  <div v-if="cycle.agentB.response" :class="proseClasses" v-html="rendered(cycle.agentB.response)" />
                  <div v-if="cycle.agentB.toolCalls.length > 0" class="space-y-1.5">
                    <ToolView v-for="tc in cycle.agentB.toolCalls" :key="tc.requestId" :tool-use="toToolUseInfo(tc)" />
                  </div>
                </template>
                <div v-if="cycle.commit" class="flex items-center gap-2 rounded-md bg-green-500/10 border border-green-500/20 px-2.5 py-1.5 text-xs">
                  ...
                  <Badge variant="outline" class="ml-auto text-[10px] shrink-0">{{ cycle.commit.filesChanged.length }} files</Badge>
                </div>
                <div v-if="cycle.summary" class="flex items-center gap-2 py-1.5">
                  ...
                  <span class="text-[10px] text-muted-foreground truncate max-w-[300px]" :title="cycle.summary">{{ cycle.summary }}</span>
                  <div class="h-px flex-1 bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
                </div>
              </template>
            </div>
      ...
      <!-- Mobile: tabbed agent switching -->
      <div v-else class="flex h-full flex-col">
        <!-- Agent toggle tabs -->
        <div class="flex items-center border-b border-border">
          <button
            class="flex min-h-[44px] flex-1 items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-medium transition-colors"
            :class="mobileAgent === 'a'
          ...
          >
            ...
            <Loader2 v-if="store.isRunning && currentCycle?.status === 'agent_a_running'" class="h-3 w-3 animate-spin" />
          </button>
          <button
            class="flex min-h-[44px] flex-1 items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-medium transition-colors"
            :class="mobileAgent === 'b'
          ...
          >
            ...
            <Loader2 v-if="store.isRunning && currentCycle?.status === 'agent_b_running'" class="h-3 w-3 animate-spin" />
          </button>
        </div>
        ...
        <div v-else ref="agentBRef" class="flex-1 overflow-y-auto p-3 space-y-3 border-l-2 border-l-emerald-500/60">
          <template v-for="cycle in store.currentRun.cycles" :key="cycle.cycleNumber">
            ...
            <div v-if="cycle.summary" class="flex items-center gap-2 py-1.5">
              <div class="h-px flex-1 bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
              <span class="text-[10px] text-muted-foreground truncate max-w-[200px]" :title="cycle.summary">{{ cycle.summary }}</span>
              <div class="h-px flex-1 bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
            </div>
          </template>
        </div>
